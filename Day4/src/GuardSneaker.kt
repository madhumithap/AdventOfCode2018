...
fun main(args: Array<String>) {
    val reposeRecords = parseRecords(getReposeRecord())
    val guards = createGuards(reposeRecords)
    val maxSleepingGuardId = findMaximumSleepingGuardId(guards)
    println("max sleeping guard id is $maxSleepingGuardId")
    val mostFrequentlySleepingGuardId = findMostFrequentlySleepingGuardId(guards)
    println("most frequently sleeping guard id is $mostFrequentlySleepingGuardId")
}

fun findMaximumSleepingGuardId(guards: Map<Int, Guard>) : Int? {
    val maxSleptGuard = guards.values.maxBy { it.totalSleptTimeInMinutes }
    maxSleptGuard?.let {
        return it.id * it.maximumSleptMinute()
    }
    return null
}

fun findMostFrequentlySleepingGuardId(guards: HashMap<Int, Guard>): Int? {
    var mostFrequentlySleptGuardId: Int? = null
    var mostFrequentlySleptMinute: Int? = null
    var maxNumberOfTimesSlept = 0
    guards.values.map { guard ->
        val frequentlySleptMinuteWithTimes = guard.mostFrequentlySleptMinuteWithTimes()
        frequentlySleptMinuteWithTimes?.let { (frequentlySleptMinute, numberOfTimesSlept) ->
            if (numberOfTimesSlept > maxNumberOfTimesSlept) {
                maxNumberOfTimesSlept = numberOfTimesSlept
                mostFrequentlySleptMinute = frequentlySleptMinute
                mostFrequentlySleptGuardId = guard.id
            }
        }
    }
    mostFrequentlySleptGuardId?.let { guardId ->
        mostFrequentlySleptMinute?.let {  minute ->
...
private fun createGuards(
    reposeRecords: List<ReposeRecord>
) : HashMap<Int, Guard> {
    val guards: HashMap<Int, Guard> = hashMapOf()
    var currentGuardId: Int? = null
    reposeRecords.forEach { reposeRecord ->
        when {
            reposeRecord.isGuardBeginningShift() -> {
                val guardIdOptional = reposeRecord.getGuardId()
                guardIdOptional?.let { guardId ->
            ...
            reposeRecord.isGuardFallingAsleep() -> {
                currentGuardId?.let {
                    guards[it]?.latestSleepStartTime = reposeRecord.date
                }
            }
            reposeRecord.isGuardWakingUp() -> {
                currentGuardId?.let {
                    guards[it]?.wakeUp(reposeRecord.date)
                }
            }
        }
    }
...
fun parseRecords(recordStatements: List<String>): List<ReposeRecord> {
    val reposeRecords = recordStatements.map {
        val dateString = it.substringAfter('[').substringBefore(']')
        val date = SimpleDateFormat("yyyy-MM-dd hh:mm").parse(dateString)
        val statement = it.substringAfter("] ")
        ReposeRecord(date, statement)
    }
    return reposeRecords.sortedBy { it.date }
}

fun getReposeRecord() : List<String> {
    return arrayListOf(
